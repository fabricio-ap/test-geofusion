...
import { Container, Header, Map, NumberInput, Table, TextInput } from '../components';
import { changeBalance, changeData, changeMarkers, changeSearch } from '../reducer';
...
export const AppContent = () => {
  ...
  useEffect(() => {
    const newData = filter.search
      ? defaultData.filter(({ name }) => name.toLowerCase().includes(filter.search.toLowerCase()))
      : defaultData;
  ...
  const onChangeBalance = ({ target }: TInputChange) => dispatch(changeBalance(target.value));
  ...
  return (
    <Fragment>
      ...
      <Container>
        <FilterContainer>
          <TextInput value={filter.search} placeholder='Pesquisar' onChange={onChangeSearch} />
          <NumberInput
